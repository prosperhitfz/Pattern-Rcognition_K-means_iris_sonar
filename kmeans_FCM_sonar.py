...
k = 2       # k为聚类的类别数
total_numbers = 208     # n为样本总个数
feature_numbers = 60      # t为数据集的特征数
...
def k_means():  # k-means算法
    ...
    # 随机选取>=0且<total_numbers的二个值作为k（2）个初始聚类中心,聚类中心为每一类的均值向量
    ...
    while 1:
        ...
        means[1] = means_copy[1]  # 初始化每代的聚类中心
        w1 = np.zeros((1, feature_numbers))
        w2 = np.zeros((1, feature_numbers))  # 初始化每一类的类别矩阵
        ...
        w2 = np.delete(w2, 0, axis=0)  # 删除原聚类中心
        means_copy[0] = np.mean(w1, axis=0)
        means_copy[1] = np.mean(w2, axis=0)
        # 更新聚类中心

        # 若两代聚类中心相同，则聚类完成，迭代停止
        if (means[0] == means_copy[0]).all() and (means[1] == means_copy[1]).all():
            print('kmeans第', epoch + 1, '次迭代：')
            print('父代和当前子代迭代设定的初始聚类中心均为\n', means[0], '\n', means[1])
            print('迭代结束，kmeans聚类分类完毕\n')
            break
        else:
            print('kmeans第', epoch+1, '次迭代：')
            print('父代迭代设定的初始聚类中心为:\n', means[0], '\n', means[1])
            print('迭代后子代更新得到的聚类中心为:\n', means_copy[0], '\n', means_copy[1], '\n')
        epoch += 1
        
        # 将每代得到的聚类结果给与标签并做标准（归一）化处理，为画图做准备
        w = np.vstack((w1, w2))  # 将全部聚类结果归并为三行的矩阵
        label1 = np.zeros((len(w1), 1))
        label2 = np.zeros((len(w2), 1))  # 初始化标签,第一类标签自动设置为0
        for i in range(len(w2)):
            label2[i, 0] = 1  # 将第二类标签设置为1
        label = np.vstack((label1, label2))
        label = np.ravel(label)  # 将矩阵000...111...222由三行变为一行（降维）
        y = label
        pca = decomposition.PCA(n_components=2)  # 60维数据降至二维
        ...
        chart.set_title("K_means_sonar_PCA")
        plt.show()
    return w1, w2, y
...
def evaluate_result_purity_kmeans(label):  # 定义purity纯度为算法性能评估准则
    ...
    correct_label1 = np.zeros((97, 1))
    correct_label2 = np.zeros((111, 1))
    for i in range(len(correct_label2)):
        correct_label2[i, 0] = 1  # 将第二类标签设置为1
    correct_label = np.vstack((correct_label1, correct_label2))
...
def FCM():  # 模糊聚类算法
    ...
    while 1:      # 循环更新隶属度矩阵U
        ...
        for j in range(k):
            for i in range(0, total_numbers):
                ...
                for loop in range(k):  # 隶属度函数（矩阵）更新公式
                    # 分母
                    U_denominator += (distance_matrix[i][j] / distance_matrix[i][loop]) ** (2 / (a - 1))
        ...
        label2 = np.zeros((len2, 1))  # 初始化标签矩阵
        for i in range(len2):
            label2[i, 0] = 1
        label = np.vstack((label1, label2))
...
data = np.mat(sonar1)  # 转换为矩阵

if __name__ == '__main__':
    W1, W2, LABEL = k_means()
    print('第一类数据个数:', W1.shape[0])
    print('第二类数据个数:', W2.shape[0])
    kmeans_acc = evaluate_result_purity_kmeans(LABEL)
